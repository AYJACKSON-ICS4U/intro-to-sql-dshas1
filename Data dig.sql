...
SELECT ID, AVG(popularity) AS avg_pop FROM Marvels;
SELECT name, MAX(popularity) AS max_pop FROM Marvels;
SELECT name, MIN(popularity) AS min_pop FROM Marvels;

SELECT name, popularity FROM Marvels GROUP BY name HAVING gender = "Female" ORDER BY popularity; 

SELECT name,
    CASE
        WHEN popularity > 90 THEN "boooo !"
        WHEN popularity > 75 THEN "bof - bof" 
        WHEN popularity > 60 THEN "not that lame"
        WHEN popularity > 45 THEN "cool"
        WHEN popularity > 30 THEN "ubber cool"
        WHEN popularity > 15 THEN "wicked"
        ELSE "ultimate" 
    END AS barometer
FROM Marvels
Order BY barometer;
...
SELECT * FROM Marvels WHERE popularity < 30 AND gender = "Female";

SELECT * FROM Marvels WHERE popularity < 30 AND alignment = "Good" OR alignment = "Neutral";
